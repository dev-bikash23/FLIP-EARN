...
const niches = ['lifestyle', 'fitness', 'food', 'travel', 'tech', 'gaming', 'fashion', 'beauty', 'business', 'education',
  'entertainment', 'music', 'art', 'sports', 'health', 'finance', 'other'];
...
if(loadingListing){
  return (
    <div className='h-screen flex items-center justify-center'>
      <Loader2Icon className='size-7 anime-spin text-indigo-600' />
    </div>
  )
}
  return (
      ...
      <form onSubmit={handleSubmit} className='space-y-8'>
        ...
        <Section title='Account Metrics'>
            <div className='grid grid-cols-1 md:grid-cols-3 gap-6 mb-6'>
              ...
              <InputField label='Monthly Views/Impression (%)' type='number' min={0} value={formData.monthly_views}
              placeholder='100000' onChange={(v) => handleInputChange('monthly_views', v)} />
            </div>
            ...
            <div className='space-y-3'>
               <CheckboxFixed label='Account is verified on the platform' checked={formData.verified} 
               onChange={(v) => handleInputChange('verified', v)}/>

               <CheckboxFixed label='Account is monetized' checked={formData.monetized} 
               onChange={(v) => handleInputChange('monetized', v)}/>
            </div>
        </Section>

      {/* PRICING */}
          <Section title='Pricing & Descripation'>
             <InputField label='Asking Price (USD) *' type='number' min={0} value={formData.price}
              placeholder='2500.00' onChange={(v) => handleInputChange('price', v)} required={true} />
       ...
       {/* IMAGES */}
          ...
          <Section title='Screenshots & Proof'>
            <div className='border-2 border-dashed border-gray-300 rounded-lg p-6 text-center'>
               ...
               <label htmlFor="images" className='px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50
               cursor-pointer'>Choose Files</label>
               <p className='text-sm tex-gray-500 mt-2'>Upload Screenshots or proof of account analytics</p>
            </div>
              ...
              <div className='grid grid-cols-2  md:grid-cols-4 gap-4 mt-4'>
                  {formData.images.map((img, index) => (
                    <div key={index} className='relative'>
                      ...
                      size-6 bg-red-600 text-white rounded-full hover:bg-red-700'>
                       x
            ...
            <div className='flex justify-end gap-3 text-sm'>
                <button onClick={() => navigate(-1)} type='button' className='px-6 py-2 border border-gray-300 rounded-lg hover:bg-50 transition-colors'>
                  Cancel
                </button>
...
const InputField = ({label, value, onChange, placeholder, type = 'text', required = false, min = null, max = null })=> (
  <div>
    ...
    className='w-full px-3 py-1.5 text-gray-600 border rounded-md focus:outline-none focus:ring-2 
...
const CheckboxFixed = ({ label, checked, onChange, required = false })=>(
...
const TextareaField = ({ label, value, onChange, required = false })=>(
  <div>
     ...
     className='w-full py-1.5 text-gray-600 border rounded-md focus:outline-none focus:ring-2
     focus:ring-indigo-500 border-gray-300' required={required} ></textarea>
  </div>
)

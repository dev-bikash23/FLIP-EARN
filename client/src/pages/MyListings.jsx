...
import { ArrowDownCircleIcon, BanIcon, CheckCircle, Clock, CoinsIcon, DollarSign, Edit, EyeIcon, EyeOffIcon, LockIcon, Plus, StarIcon, TrashIcon, TrendingUp, Users, WalletIcon, XCircle } from "lucide-react"; 
...
const getStatusIcon = (status)=>{
    switch (status) {
      ...
      case "ban":
      return <BanIcon className="size3.5" />;
      case "sold":
      return <DollarSign className="size3.5" />;
      case "inactive":
      return <XCircle className="size3.5" />;
      default:
...
return (
    <div className='px-6 md:px-16 lg:px-24 xl:px-32 pt-8'>
        ...
        <div className='flex flex-col md:flex-row justify-between items-start md:items-center mb-8'>
            <div>
             ...
             <button onClick={()=>navigate('/create-listing')} className='bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded font-medium 
        ...
        <div className='grid grid-cols-1 md:grid-cols-4 gap-6 mb-8'>
            <StatCard title='Total Listings' value={userListings.length} 
            icon={<Eye className='size-6 text-indigo-600' />} color='indigo'/>

            <StatCard title='Active Listings' value={activeListings} 
            icon={<CheckCircle className='size-6 text-green-600' />} color='green'/>

            <StatCard title='Sold' value={soldListings} 
            icon={<TrendingUp className='size-6 text-indigo-600' />} color='indigo'/>

            <StatCard title='Total Value' value={`${currency}${totalValue.toLocaleString()}`} 
            icon={<DollarSign className='size-6 text-yellow-600' />} color='yellow'/>
        </div>
        
        {/* Balance Section */}
        <div className='flex flex-col sm:flex-row justify-between gap-4 xl:gap-20 p-6 mb-10 bg-white rounded-xl
             ...
             {label: 'Withdrawn', value: balance.withdrawn, icon:ArrowDownCircleIcon},
              {label: 'Available', value: balance.available, icon: CoinsIcon},
               ...
               <div key={index} className='flex flex-1 items-center justify-between p-4 rounded-lg border border-gray-100 cursor-pointer'>
                  <div onClick={()=>item.label === "Available" && setShowWithdrawl(true)} className='flex items-center gap-3'>
        ...
        {userListings.length === 0 ? (
          <div className='bg-white rounded-lg border border-gray-200 p-16 text-center'>
             ...
             <button onClick={()=> navigate("/create-listing")} className='bg-indigo-600 hover:bg-indigo-700
              text-white px-6 py-2 rounded-lg font-medium'>Create First Listing</button>
          </div>
      )
      :  
     (
                       ...
                       <div className='flex items-center justify-between pt-3 border-t border-gray-200'>
                        <span className='text-2xl font-bold text-gray-800'>
                          ...
                          <div className='flex items-center space-x-2'>
                             ...
                             <button onClick={()=>toggleStatus(listing.id)} className='p-2 border border-gray-300 rounded-lg hover:bg-gray-50 hover:text-purple-800'>
    ...
    {showWithdrawl && (
  <WithdrawModal onClose={() => setShowWithdrawl(false)} />
)}

     {/* Footer */}
         <div className='bg-white border-t border-gray-200 p-4 text-center mt-28'>
             <p className='text-sm text-gray-500'>
              @2025 <span className='text-indigo-600'>SocioSell</span>. All rights reserved.
             </p>
         </div>
    </div>
  )

...
const filteredChats = useMemo(()=>{
  ...
  return chats.filter((chat)=>{
    ...
    return chat.listing?.title?.toLowerCase().includes(query) || chatUser?.name?.
        ...
        {loading ? (
          ...
          <div className='bg-white rounded-lg shadow-xs border border-gray-200 p-16 text-center'>
             ...
             <h3 className='text-xl font-medium text-gray-800 mb-2'>
              {searchQuery ? 'No chats found' : 'No messages yet'}</h3>
              ...
              key={chat.id} className='w-full p-4 hover:bg-gray-50 transition-colors text-left'>
                 <div className='flex items-start space-x-4'>
                   ...
                   <div className='flex-1 min-w-0'>
                      ...
                      <p className={`text-sm truncate ${!chat.isLastMessageRead && 
                      chat.lastMessageSenderId !== user?.id ? 'text-purple-600 font-medium' : "text-gray-500" }`}>

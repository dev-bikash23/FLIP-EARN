...
const CredentialSubmission = ({ onClose, listing}) => {
  ...
  const data = credential.reduce((acc, c) => {
    acc[c.name] = c.value;
    return acc;
  }, {});
  ...
  return (
    <div className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-50 flex items-center justify-center sm:p-4'>
       ...
       {/* Header */}
       <div className='bg-gradient-to-r from-indigo-700 to-indigo-400 text-white p-4 sm:rounded-t-lg flex items-center justify-between'>
       ...
       {/* Form */}
       <form onSubmit={handelSubmission} className='flex flex-col items-start gap-4 p-4 overflow-y-scroll'>
         {credential.map((cred, index)=>(
          <div key={index} className='grid grid-cols-[2fr_3fr_1fr] items-center gap-2'>
              ...
              <input type='text' value={cred.value} onChange={(e)=>setCredential((prev)=>prev.map
                ((c, i)=>(i === index ? {...c, value: e.target.value} : c)))} className='w-full px-2 py-1.5
        ...
        <div className="flex items-center gap-2">
            <input type="text" value={newField} onChange={(e)=>setNewField(e.target.value)} placeholder="field name ..." 
